...
//https://riptutorial.com/javafx/example/11261/extending-rectangle-adding-new-stylable-properties
//custom node
public class SupremeRectangle extends Rectangle {
    ...
    private int state;
    private int rid;//rectangle id as number in arraylist
    private String ridAsCoor;//rectangle id as letter and number in matrix ex. A2
    ...
    public void setRid(int state) {
        this.rid = rid;
    }//w konstruktorze przypisanko to w sumie niepotrzebne

    public String getRidAsCoor() {
        return ridAsCoor;
    }

    public void setRidAsCoor(String XY) {
...
//    }
    ...
    public SupremeRectangle(double x, double y, double width, double height, int state, int rid, String ridAsCoor) {
        super(x, y, width, height);
        this.state = state;
        this.rid = rid;
        this.ridAsCoor = ridAsCoor;
    ...
    private final static CssMetaData<SupremeRectangle, Number> WIDTH_META_DATA = new CssMetaData<SupremeRectangle, Number>("-fx-width", StyleConverter.getSizeConverter()) {

        @Override
        public boolean isSettable(SupremeRectangle styleable) {
            // property can be set iff the property is not bound
            return !styleable.styleableWidth.isBound();
        }

        @Override
        public StyleableProperty<Number> getStyleableProperty(SupremeRectangle styleable) {
    ...
    // use converter for numbers
    private final static CssMetaData<SupremeRectangle, Number> HEIGHT_META_DATA = new CssMetaData<SupremeRectangle, Number>("-fx-height", StyleConverter.getSizeConverter()) {
